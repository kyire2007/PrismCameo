...
def recolorRC(src, dst):
    ...
    Blue and green are replaced with cyans.
...
class BGRPortaCurveFilter(BGRCurveFilter):
    """A filter that applies Porta-like curve to BGR"""

    def __init__(self, dtype = numpy.uint8):
        BGRCurveFilter.__init__(
            self,
            vPoints = [(0,0), (23,20), (153,173), (255,255)],
...
class BGRCrossProcessCurveFilter(BGRCurveFilter):
    """A filter that applies cross-process-like curve to BGR"""
...
class FindEdgesFilter(VConvolutionFilter):
    ...
    def __init__(self):
        kernel = numpy.array([[-1, -1, -1],
                              [-1, 8, -1],
                              [-1, -1, -1]])
        VConvolutionFilter.__init(self, kernel)
